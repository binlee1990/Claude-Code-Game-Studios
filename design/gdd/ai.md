...
> **Status**: Implemented — MVP NullAI baseline + Tier 2 BasicAI runtime execution
...
|------|----------|------------|-------|
| `ai_controller` (实现类引用) | Game 场景 / TurnManager DI | `NullAI` / `BasicAI` / 任何 AIController 子类 | 通过依赖注入替换。默认 `Game` 场景固定为 `NullAI`；BasicAI 已可被 TurnManager runtime 执行 |
