...
> **Status**: Implemented — MVP NullAI baseline + Tier 2 BasicAI runtime execution and mode selection
...
AI / AIController 是 SRPG 骨架中的**敌方行为驱动器**：它定义了一个可插拔的控制器接口 —— `AIController.take_turn(units, world_state) -> ActionList` —— Turn System 在每个 ENEMY 阶段调用此接口，AI 返回一份动作列表（移动目标 + 攻击目标），Turn System 持有执行权。MVP 默认交付 `NullAI` 实现（空动作列表，因为热座模式下玩家手动控制双方）；Tier 2 已交付 `BasicAI`（最近目标启发式）和 Turn-layer ActionList 执行。NullAI 证明接口的"零行为"端；BasicAI 证明接口的"有行为"端。默认 `Game` 场景仍使用 NullAI 以保留已签核的热座 baseline，BasicAI 可通过依赖注入启用。
...
10. **BasicAI 行为规约（Tier 2，已实现）**: 为证明 AIController 接口的正确性，此处定义并实现 BasicAI 的当前行为：
...
该行为规约证明 AIController 接口容纳 NullAI 和 BasicAI 两种互不相同的实现。BasicAI 仍保持纯计划生成器，不导入 TurnManager；Turn-layer 执行由 `AIActionExecutor` 和 `TurnManager` 负责。
...
| **BasicAI** (Tier 2) | Hard | `AIController` 基类 + `ActionPlan` / `ActionList` / `WorldState` 类型 | BasicAI 通过继承 AIController 实现新的行为。BasicAI 不导入 TurnManager；Turn-layer 负责执行其 ActionList |
...
|------|----------|------------|-------|
| `ai_controller` (实现类引用) | Game 场景 / TurnManager DI | `NullAI` / `BasicAI` / 任何 AIController 子类 | 通过依赖注入替换。默认 `Game` 场景通过 `srpg_mini/enemy_ai_mode="hotseat"` 使用 `NullAI`；`basic` 或 `--enemy-ai=basic` 启用 BasicAI |
...
**AC-AI-017 — Interface admits BasicAI implementation** [Structural]
GIVEN `src/ai/basic_ai.gd` (extends AIController, overrides take_turn()), WHEN source is inspected, THEN BasicAI depends on MovementResolver and AttackRangeResolver but does NOT import or reference TurnManager. Verified by automated source inspection in `tests/unit/ai/basic_ai_test.gd`.
