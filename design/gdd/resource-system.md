...
- ✅ **EventBus GDD** §Interactions 列出"资源系统 — 发布 `resource.{id}.changed`"——一致；本 GDD 额外引入 `cap_changed` 和 `overflow` 两个事件，需在 EventBus GDD §11 命名空间约定中追加
...
| EventBus GDD §11 命名空间约定中需追加 `cap_changed` 和 `overflow` 两个新事件 | 开发者 | 实现阶段前 | ✅ 已解决 2026-05-03 — event-bus.md §Core Rules 第 11 条已追加 `resource.{id}.cap_changed` 和 `resource.{id}.overflow` 命名空间 |
