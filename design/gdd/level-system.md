...
8. **存档/读档时 modifier 重建**：本系统订阅 `EventBus` 的 `save.loaded` 事件（save-system.md §Detailed Design 行 89 确认）。重建流程：
   ...
   - 注：attribute-system.md §Edge Cases 行 393 提到的 `save.restored` 是历史笔误（应为 `save.loaded`），需在 cross-consistency 阶段一并修订

9. **事件发布**：
   - **`level.changed`**：payload `{entity_id: String, old_level: int, new_level: int, levels_gained: int}`——单次 gain_exp 内**只发 1 条**（聚合），无论连升 1 级还是 100 级。**注意**：相对 event-bus.md §Interactions 行 123 已锁定的 `{old_level, new_level}` payload，本 GDD 扩展加入 `entity_id` 与 `levels_gained` 字段；需在 cross-consistency 阶段同步更新 event-bus.md。
   - **`realm.advanced`** (新增事件)：payload `{entity_id: String, old_realm: String, new_realm: String}`——跨境界时发布（连跳跨多境界仍只发 1 条，old_realm 为起始境界、new_realm 为终态境界）。需在 cross-consistency 阶段在 event-bus.md §Core Rules 11 命名空间约定 + §Interactions 表追加。
...
- ⚠ **EventBus GDD** §Interactions 行 123 列出 `level.changed` payload 为 `{old_level, new_level}`——本 GDD 扩展为 `{entity_id, old_level, new_level, levels_gained}`，需在 `/consistency-check` 阶段更新 event-bus.md 行 123 的 payload 描述
- ⚠ **EventBus GDD** §Core Rules 11 命名空间约定中**未列出** `realm.advanced` 事件——需在 `/consistency-check` 阶段追加 `realm.advanced` 命名空间 + 在 §Interactions 表追加等级系统发布 `realm.advanced` 行
- ⚠ **AttributeSystem GDD** §Edge Cases 行 393 提到 `EventBus.emit("save.restored", {})`——这是历史笔误，实际事件名为 `save.loaded`（save-system.md §Detailed Design 行 89 确认）；需在 `/consistency-check` 阶段一并修订
...
- ⚠ **EventBus GDD** §Interactions 行 123 `level.changed` payload 锁定为 `{old_level, new_level}`，本 GDD 扩展为 `{entity_id, old_level, new_level, levels_gained}`——需 `/consistency-check` 修订
- ⚠ **EventBus GDD** §Core Rules 11 命名空间未列出 `realm.advanced`——需 `/consistency-check` 追加
- ⚠ **AttributeSystem GDD** §Edge Cases 行 393 `save.restored` 是历史笔误（应为 `save.loaded`，save-system.md §Detailed Design 行 89 确认）——需 `/consistency-check` 修订
...
|----------|-------|----------|-----------|
| EventBus GDD §Interactions 行 123 `level.changed` payload 已锁定为 `{old_level, new_level}`，本 GDD 扩展为 `{entity_id, old_level, new_level, levels_gained}`——需在 `/consistency-check` 阶段同步更新 event-bus.md 行 123 的 payload 描述与 §Core Rules 11 命名空间约定 | 设计师 | `/consistency-check` 阶段 | — |
| EventBus GDD §Core Rules 11 命名空间约定中**未列出** `realm.advanced` 事件——需在 `/consistency-check` 阶段追加 `realm.advanced` 命名空间 + 在 §Interactions 表追加等级系统发布 `realm.advanced` 行 | 设计师 | `/consistency-check` 阶段 | — |
| AttributeSystem GDD §Edge Cases 行 393 提到 `EventBus.emit("save.restored", {})` 是历史笔误——实际 save-system.md §Detailed Design 行 89 使用 `save.loaded`；需在 `/consistency-check` 阶段一并修订 attribute-system.md 该行表述 | 设计师 | `/consistency-check` 阶段 | — |
