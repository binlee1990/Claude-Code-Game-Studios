...
| **Tier 2（垂直切片）** | 可配置地图 + 1 种地形类型 | `BasicAI`（最近目标启发式）、runtime ActionList 执行、AI mode selection、Map Variant Pack、runtime map selection、rough terrain movement cost 已完成；剩余为简易职业三角 | MVP 之后，增量添加 |
...
- [x] Tier 2 下一步：Sprint 9 `Weighted Terrain Movement` —— `R` rough terrain 可行走但 cost=2，MovementResolver 与 BasicAI 均按总移动代价行动。
