...
**关于 systems-index 依赖声明**：systems-index 中公式引擎的 "Depends On" 列为"大数值系统, 随机数与种子系统"。这表示架构协作前置，而不是 FormulaEngine 直接导入这两个模块。DataConfig 是 Post-MVP 公式来源，不进入 MVP 硬依赖列。

**双向一致性**：大数值系统 GDD 的 Interactions 表已列出公式引擎为双向关系；随机数与种子系统 GDD 的 Interactions 表已列出公式引擎为下游消费。修正器/倍率、等级、战斗、掉落、产出/存储等下游 GDD 均已声明对 FormulaEngine 的使用边界。
...
|----------|-------|----------|------------|
| Godot 4.6 的 `Expression` 自定义函数注入方式是否满足 `softcap_*` 等函数需求？ | 开发者 | 实现阶段前 | 保留：需用最小原型验证 API 行为。 |
| 公式 ID 是否统一采用命名空间前缀（如 `combat.damage_mult`）以避免配置冲突？ | 设计师 + 开发者 | 首批公式配置前 | 保留：MVP 可先用少量短 ID；扩展前需定命名规范。 |
| `evaluate_raw()` 是否应在生产构建中禁用，仅保留调试/测试用途？ | 技术总监 | 实现阶段前 | 保留：涉及安全面和调试便利，需要实现策略决定。 |
