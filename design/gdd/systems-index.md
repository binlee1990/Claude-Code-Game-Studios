...
4. **修正器/倍率引擎** — depends on: 公式引擎, 大数值系统, 事件总线
5. **存档系统** — depends on: 数据配置系统, 时间管理器, 事件总线

### Core Gameplay Layer (depends on Foundation + Core Data)

1. **资源系统** — depends on: 大数值系统, 事件总线, 数据配置系统
2. **属性系统** — depends on: 大数值系统, 修正器/倍率引擎, 事件总线, 数据配置系统
3. **物品/材料系统** — depends on: 数据配置系统, 大数值系统
4. **产出乘数系统** — depends on: 修正器/倍率引擎, 数据配置系统, 事件总线, 大数值系统
5. **调试控制台** — soft-depends on: 事件总线, 数据配置系统, 资源系统, 属性系统, 修正器/倍率引擎, 产出乘数系统, 时间管理器, 存档系统, 数值格式化系统, 大数值系统
...
| MVP First Playable Achieved | 2026-05-05 via Sprint 11 UI Scene Layer completion + `production/qa/evidence/sprint-11/first-playable-smoke.md` + `reports/report_21/results.xml`（完整 HUD shell、5 MVP 主屏、Toast、Offline Drawer、Settings/Confirm/Stance modals、Debug Console UI） |
