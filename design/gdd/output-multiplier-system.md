...
3. **配置驱动**：启动时通过 `DataConfig.get("production_config")` 加载 `production_config.json`，缓存在内部 `base_rates: Dictionary[String, BigNumber]` 和 `resource_configs: Dictionary[String, Dictionary]` 中。配置格式：
   ```json
   {
     "lingqi": {
   ...
   - `base_rate_per_second` 的字符串格式由 BigNumber 解析（支持 `"1.0"`, `"1e3"` 等）
...
6. **叠加模型**：完全委托给 ModifierEngine 的三阶段管线：
   ```
   final_rate = (base_rate + add_sum) × Π pool_mult(pool)  for pool ∈ {realm, equipment, zone, buff}
   ```
   这意味着：
   ...
   - `add_sum` 为 0 除非有来源注册了 ADD 类型修正（MVP 预计无——所有产出来源均为 MULT 类型）
...
8. **消费者查询 API**（供 AutoProduction / Cultivation / HUD 调用）：
   ```
   get_production_rate(resource_id: String) → BigNumber
   # 返回该资源每秒最终产出量（base_rate × total_multiplier）
   # allows_passive=false → 返回 BigNumber.ZERO
   # 不存在 resource_id → 返回 BigNumber.ZERO + warning
   # 内部：base_bn = base_rates[resource_id]; return ModifierEngine.apply(target, base_bn)

   get_tick_amount(resource_id: String, delta_seconds: float) → BigNumber
   # 便捷方法：get_production_rate(resource_id) × delta_seconds
   ...
   # 返回详细分解（供调试面板/工具提示）
   # {
   #   "base_rate": BigNumber,           # 基础每秒产出
   #   "add_sum": float,                 # 加法修正总和
   #   "pools": {                        # 每池倍率
   #     "realm": 2.0,
   #     "equipment": 1.25,
   #     "zone": 1.10,
   ...
   #   "rate_per_second": BigNumber      # get_production_rate() 的值
   # }
   ```

9. **初始化与生命周期**：
   - `_ready()`: assert `DataConfig != null` 且 `ModifierEngine != null`，调用 `load_config()` 加载 `production_config.json` 并缓存
   - 启动时**不预先注册任何 modifier**——所有来源在上层系统激活时才注册。这意味着如果没有装备/境界/区域/Buff 系统注册任何修正器，`get_multiplier()` 返回 `1.0`，`get_production_rate()` 返回纯 base_rate
...
13. **性能预算**：
    - 单次 `get_production_rate()` 耗时：~0.007 ms（base_rate Dict lookup + ModifierEngine cache hit + BigNumber multiply_float）
    - 每 tick（≥0.5s）对 4 个被动资源各查询一次 = 4 × 0.007 = 0.028 ms，远低于帧预算
...
| **大数值系统** | 上游依赖 | 硬依赖 | `base_rate` 存储为 BigNumber；`get_production_rate()` 返回 BigNumber；`get_tick_amount()` 内部调用 `BigNumber.multiply_float()` |
...
| **自动产出系统** | 下游消费 | 硬依赖 | 每 tick 调用 `OMS.get_tick_amount(id, delta)` → 将结果传入 `ResourceSystem.add(id, amount)` |
| **修炼系统** | 下游消费 | 硬依赖 | 调用 `OMS.get_tick_amount("lingqi", delta)` 和 `OMS.get_tick_amount("xiuwei", delta)` 获取修炼产出 |
...
| **离线收益结算系统** | 下游 → 查询 | 软依赖 | 离线结算时调用 `get_production_rate()` 获取当前（或离线时间点）的产出速率，用于批量计算离线收益 |
...
- ⚠️ EventBus GDD §Core Rules 11 命名空间约定中需追加 `"production_multiplier_changed"` 事件——建议在 `/consistency-check` 阶段补充
...
| 公式 | 所有权 | 定义位置 |
|------|-------|---------|
| `P_rate` — 最终每秒生产速率 | **OMS 拥有** | 本 GDD Formula 1 |
...
| 委托公式 | 定义位置 | OMS 使用方式 |
|---------|---------|------------|
| `add_sum(target) = Σ v_i` | modifier-engine.md §Formula 1 | MVP 恒为 0.0（无 ADD 类型产出来源），通过 `apply()` 隐式使用 |
| `pool_mult(target, pool) = 1.0 + Σ v_j` | modifier-engine.md §Formula 2 | 仅在 `get_breakdown()` 中通过 `get_pool_multiplier()` 查询 |
| `final_mult(target) = Π pool_mult(target, pool)` | modifier-engine.md §Formula 3 | `get_multiplier()` 直接委托；`get_production_rate()` 通过 `apply()` 隐式使用 |
...
`P_rate` 是本系统核心公式——将 `production_config.json` 中的基础速率与 ModifierEngine 返回的最终乘数结合。

```
P_rate(resource_id) = ModifierEngine.apply("{resource_id}_production", base_rate)
```

ModifierEngine 内部执行 `(base_rate + add_sum) × final_mult`。MVP 阶段所有产出来源均为 MULT 类型，`add_sum = 0`，因此等价于：

```
P_rate = base_rate × final_mult
```
...
| base_rate | b | BigNumber | `[ZERO, config_max]` | 从 `production_config.json` 加载的基础每秒产出。lingqi=1.0, xiuwei=0.1, lingshi=0.1, herb=0.02 |
| target | — | String | `"{id}_production"` | ModifierEngine 查询键，例 `"lingqi_production"` |
| add_sum | a | float | `0.0`（MVP） | ADD 类型修正总和。MVP 无 ADD 来源，恒为 0 |
| final_mult | f | float | `[0.0, +∞)` | 所有池倍率的连乘积。由 ModifierEngine 计算并缓存（dirty-flag 模式）。无修正时 = 1.0 |
| P_rate | P | BigNumber | `[ZERO, MAX]` | 该资源每秒最终产出量 |

**输出范围：** `[BigNumber.ZERO, BigNumber.MAX]`。

**工作示例（灵气，中等境界，4 池全激活）：**

输入：
- `resource_id = "lingqi"`, `base_rate = BigNumber.from_float(1.0)`
...
P_rate = BigNumber.from_float(1.0) × 3.30 = BigNumber.from_float(3.30)
```

结果：灵气每秒产出 **3.30**。

**`allows_passive = false` 时的行为**：`get_production_rate("exp")` 无视 ModifierEngine 状态，直接返回 `BigNumber.ZERO`——战斗经验走战斗掉落管线，不参与被动产出乘数。

---

### Formula 2: Tick 产出量 (Tick Amount)

将每秒速率转换为离散时间片的产出量。AutoProduction 和 Cultivation 的主调用口。

```
A_tick(resource_id, delta) = P_rate(resource_id).multiply_float(delta)
```

**前置条件**：`delta > 0`；否则返回 `BigNumber.ZERO`。
...
| P_rate | P | BigNumber | `[ZERO, MAX]` | 来自 Formula 1 的每秒产出速率 |
| A_tick | A | BigNumber | `[ZERO, MAX]` | 本 tick 应传递给 `ResourceSystem.add()` 的产出量 |

**输出范围：** `[ZERO, BigNumber.MAX]`。

**delta 边界处理：**
- `delta ≤ 0`：返回 ZERO，打印 warning。时间不应倒流或停滞。
- `delta < 1e-6`（< 1μs）：BigNumber 归一化可能钳位到 ZERO。正常游戏中不会发生（tick ≥ 0.5s）。
- `delta 无上限`：离线结算系统可传入任意时长（如 8h = 28800s），BigNumber 可处理。
- 浮点 delta 精度：≥ 10^7 秒时 float 精度开始丢失最低有效位，但对游戏数值无实际影响。

**工作示例（灵气，delta = 0.5s，续上例）：**

```
P_rate("lingqi") = BigNumber.from_float(3.30)
A_tick = BigNumber.from_float(3.30).multiply_float(0.5) = BigNumber.from_float(1.65)
```
...
```
P_rate("lingshi") = BigNumber.from_float(0.1) × 3.30 = BigNumber.from_float(0.33)
delta = 1800.0  (30 min)
A_tick = BigNumber.from_float(0.33).multiply_float(1800.0) = BigNumber.from_float(594.0)
```
...
```
T_query = T_lookup + T_ME_cache + T_BN_mult
```
...
| BigNumber 乘法 | T_BN_mult | float | [0.001, 0.010] | `multiply_float(final_mult)` + 归一化 |
| **单次查询总耗时** | **T_query** | **float** | **[0.003, 0.018]** | 缓存命中典型值 ~0.005 ms |

**输出范围：** `[0.003, 0.018]` ms（缓存命中）。
...
**公式边界情况：**
- **`base_rate = ZERO`**（如 exp 配置为 `"0"`）：`P_rate = ZERO`，`A_tick = ZERO`。合法——`allows_passive=false` 的资源基础速率为 0。
- **`final_mult = 0.0`**（所有池被钳位到 0）：`P_rate = ZERO`。极不可能但数学可达到。
- **`delta` 极小（< 1e-6 秒）**：BigNumber 归一化可能钳位到 ZERO。返回 ZERO 合理——微小时间片不应产生可测量产出。
- **`P_rate` 已为 `BigNumber.MAX`**：`multiply_float(delta)` → delta > 1 时饱和为 MAX；delta < 1 时结果变小。正常游玩不会触及。

## Edge Cases

### 初始化与依赖不可用

- **If ModifierEngine 在 `get_production_rate()` 调用时尚未初始化**：返回 `BigNumber.ZERO`，打印 warning `"ModifierEngine not available for production rate query: {resource_id}"`。`_ready()` 的 assert 覆盖正常启动路径，但消费者系统可能在 Autoload 初始化顺序异常时提前调用——安全降级优于崩溃。首几帧的产出查询不产生数值。
...
- **If `base_rate_per_second` 不是合法的 BigNumber 字符串**（如 `"abc"`, `""`）：`BigNumber.from_string()` 返回 ZERO。OMS 将该资源的 `base_rate` 设为 ZERO、`allows_passive` 强制设为 false，打印 error `"Invalid base_rate_per_second for '{id}': '{raw}' — resource disabled"`。
...
- **If `delta` 极大（如 7 天离线 = 604800 秒）**：OMS 本身不钳位 delta。`BigNumber.multiply_float(604800.0)` 可精确处理——浮点 delta 在 ~10^7 秒级别开始丢失精度（~3 位十进制数字），对游戏数值无实际影响。若离线结算需要 > 10^7 秒的精度，应由离线结算系统自行分批处理。
- **If `get_production_rate()` 返回 ZERO 但 `base_rate > ZERO`**（所有池被钳位到 0.0）：打印 warning `"All pools clamped to zero for '{resource_id}' — production halted. Check for excessive debuffs"`。极不可能发生（需所有 4 池的 Σ v < -1.0），但一旦发生则表示配置错误或极端减益叠加。
- **If `get_breakdown()` 对未配置的 resource_id 调用**：返回 `{"base_rate": BigNumber.ZERO, "add_sum": 0.0, "pools": {}, "final_multiplier": 1.0, "rate_per_second": BigNumber.ZERO}`。与 `get_production_rate()` 降级行为一致。
...
- **If `get_tick_amount()` 的 delta 参数为浮点极值**（如 `INF` 或 `NAN`）：BigNumber 层将 INF/NAN 钳位为 ZERO → `A_tick = ZERO`。返回 ZERO，打印 warning。
...
| **大数值系统** (BigNumber) | 硬依赖 | `base_rate` 存储为 BigNumber；`get_production_rate()` 返回 BigNumber；`get_tick_amount()` 内部调用 `BigNumber.multiply_float()` |
...
| 参数 | 类型 | 默认值 | 安全范围 | 调整影响 |
|------|------|--------|---------|---------|
| `base_rate_per_second` | BigNumber 字符串 | 见下表 | `["0", "1e30"]` | 直接影响该资源的产出速度。过高导致玩家过早撞上限（At Cap 频繁触发 overflow）；过低使该资源严重稀缺，阻塞依赖它的所有升级 |
...
- [ ] **AC-01** **GIVEN** `production_config.json` 包含 5 资源定义（lingqi base `"1.0"` allows_passive=true, xiuwei base `"0.1"` allows_passive=true, lingshi base `"0.1"` allows_passive=true, herb base `"0.02"` allows_passive=true, exp base `"0"` allows_passive=false），**WHEN** `OutputMultiplierSystem._ready()` 执行完毕，**THEN** `get_production_rate("lingqi")` 返回 `BigNumber.from_float(1.0)`，`get_production_rate("xiuwei")` 返回 `BigNumber.from_float(0.1)`，`get_production_rate("lingshi")` 返回 `BigNumber.from_float(0.1)`，`get_production_rate("herb")` 返回 `BigNumber.from_float(0.02)`，`get_production_rate("exp")` 返回 `BigNumber.ZERO`

- [ ] **AC-02** **GIVEN** exp 的 `allows_passive = false`，且 target `"exp_production"` 下存在 realm modifier（value=1.0, MULT, pool="realm"），**WHEN** 调用 `get_production_rate("exp")`，**THEN** 返回 `BigNumber.ZERO`（无视 modifier），且 `get_tick_amount("exp", 10.0)` 返回 `BigNumber.ZERO`，且 `activate_source({resource_id: "exp", source_type: "equipment", value: 0.15, source_id: "exp_src"})` 返回 `""` 并打印 warning

- [ ] **AC-03** **GIVEN** `DataConfig.get("production_config")` 返回空 Dictionary（配置缺失或不可解析），**WHEN** `load_config()` 执行，**THEN** 系统以零资源、空 base_rates 初始化，`get_production_rate("lingqi")` 返回 `BigNumber.ZERO`，`activate_source({...})` 对任何 resource_id 返回 `""`，不崩溃

### Activation and Source Registration

- [ ] **AC-04** **GIVEN** target `"lingqi_production"` 下无任何 modifier（`get_multiplier("lingqi")` 返回 `1.0`），**WHEN** 调用 `activate_source({resource_id: "lingqi", source_type: "equipment", value: 0.15, source_id: "equip_ring_001"})`，**THEN** 返回非空 modifier ID 字符串，且 `get_multiplier("lingqi")` 返回 `1.15`，且 `get_production_rate("lingqi")` 返回 `BigNumber.from_float(1.15)`
...
- [ ] **AC-09** **GIVEN** target `"lingqi_production"` 下无任何 modifier，**WHEN** 调用 `get_production_rate("lingqi")`，**THEN** 返回 `BigNumber.from_float(1.0)`（base_rate × 1.0）

- [ ] **AC-10** **GIVEN** lingqi 下 4 个来源均激活：realm value=1.0（池倍率 2.0）、equipment 两件 value=0.15+0.10（池倍率 1.25）、zone value=0.10（池倍率 1.10）、buff value=0.20（池倍率 1.20），总 final_mult = 2.0 × 1.25 × 1.10 × 1.20 = 3.30，**WHEN** 调用 `get_production_rate("lingqi")`，**THEN** 返回 `BigNumber.from_float(3.30)`

- [ ] **AC-11** **GIVEN** `get_production_rate("lingshi")` 返回 `BigNumber.from_float(0.33)`（base 0.1 × multiplier 3.30），**WHEN** 调用 `get_tick_amount("lingshi", 0.5)` 和 `get_tick_amount("lingshi", 1800.0)`，**THEN** 分别返回 `BigNumber.from_float(0.165)` 和 `BigNumber.from_float(594.0)`
...
- [ ] **AC-13** **GIVEN** realm（value=1.0, source_id="realm_lianqi"）和 equipment（value=0.25, source_id="equip_ring"）对 lingqi 生效，base_rate = `BigNumber.from_float(1.0)`，**WHEN** 调用 `get_breakdown("lingqi")`，**THEN** 返回 Dictionary 其中 `base_rate` = `BigNumber.from_float(1.0)`，`add_sum` = `0.0`，`pools.realm` = `2.0`，`pools.equipment` = `1.25`，`pools.zone` = `1.0`（空池），`pools.buff` = `1.0`（空池），`final_multiplier` = `2.50`，`rate_per_second` = `BigNumber.from_float(2.50)`
...
- [ ] **AC-16** **GIVEN** equipment modifier `source_id: "equip_ring_001"`（value=0.15）对 lingqi 生效，`get_multiplier("lingqi")` = `1.15`，**WHEN** 调用 `deactivate_source("equip_ring_001")`，**THEN** 返回 `1`（一个 modifier 被移除），且 `get_multiplier("lingqi")` 返回 `1.0`，且 `get_production_rate("lingqi")` 返回 `BigNumber.from_float(1.0)`
...
- [ ] **AC-20** **GIVEN** `get_production_rate("lingqi")` 返回非零 BigNumber，**WHEN** `get_tick_amount("lingqi", 0.0)` 和 `get_tick_amount("lingqi", -1.0)` 分别被调用，**THEN** 两者均返回 `BigNumber.ZERO`，各打印一条 warning 提示 delta 无效
...
| EventBus GDD §Core Rules 11 命名空间约定中需追加 `"production_multiplier_changed"` 事件 | 开发者 | `/consistency-check` 阶段 | — |
