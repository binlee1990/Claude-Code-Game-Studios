...
5. **查询语义与不变量**：
   ...
   - **MVP 即时扫描**：`query_by_item_class` / `query_by_tag` 用 O(N) 扫描；预构建倒排索引推迟到 Alpha（代码注释标记 `# TODO Alpha: build inverted index when item count > 50`）
...
- ⚠ **EventBus GDD** §Core Rules 11 命名空间约定**未列入** `item_registry.loaded` / `item_registry.reloaded` 两个 lifecycle 事件——需在 `/consistency-check` 阶段追加（与 resource-system / attribute-system 同模式）
...
| **EventBus GDD §Core Rules 11** 命名空间约定**未列入** `item_registry.loaded` / `item_registry.reloaded` 两个 lifecycle 事件。需在 `/consistency-check` 阶段追加（与 resource-system / attribute-system 同模式） | 开发者 | `/consistency-check` 阶段 | ✅ 已解决 2026-05-03 — event-bus.md §Core Rules 11（L75-76）已追加两条 item_registry 命名空间，含 payload 说明 |
