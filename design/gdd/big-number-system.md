...
GDScript 原生的 `int`（64-bit，上限约 9.2e18）和 `float`（double，整数精度在 2^53 后丢失）无法满足修仙放置游戏中灵气、修为、战力、掉落倍率等数值在后期的指数级膨胀需求——本系统填补这一空白，确保绝对量在任意阶段不丢失精度。本系统仅表示 ≥ 1 的绝对量值；比值（0~1 范围）和百分比乘数由公式引擎使用 `float` 处理。

玩家不直接与此系统交互。它是所有资源显示、战斗计算、成长公式和离线模拟的底层依赖——7 个系统直接依赖它，间接依赖贯穿全部 30 个 MVP 系统。没有它，游戏的中后期数值体系将无法表达。

## Player Fantasy

玩家不会"看到"大数值系统，但他们会感受到一个没有天花板的成长世界。当修为从百位跳到万亿、从万亿跳到科学计数法（那个"1.23e15 首次出现在屏幕上"的时刻）、从科学计数法跳到更高层级的数值表达——每一次数位跃升都是修仙世界观中"破境飞升"的量化映射。数字的无限膨胀让"凡人成长为诸天至尊"不再只是一句文案，而是玩家亲身经历的数值体验。系统永远不会告诉你"你已经到顶了"。
...
4. **运算规则**：
   ...
   **幂运算 `power(a, n)`**（n 为 float）：
   - `total_log = log10(a.mantissa) + a.exponent`
   - `result_e = floor(total_log * n)`
   - `result_m = 10 ^ (total_log * n - result_e)`，归一化
...
5. **API 表面**：
   - 算术方法：`add()`, `subtract()`, `multiply()`, `multiply_float()`, `divide()`, `power()`, `log10()`
   - 比较方法：`equals()`, `less_than()`, `greater_than()`, `less_or_equal()`, `greater_or_equal()`, `compare()`
   - 工厂方法：`BigNumber.from_int()`, `BigNumber.from_float()`, `BigNumber.from_string()`, `BigNumber.from_dict()`, `BigNumber.zero()`
   - 转换方法：`to_int()`, `to_float()`, `to_string()`, `to_dict()`
   - 查询方法：`is_zero()`, `is_max()`, `magnitude()`（返回 exponent）
   - 工具方法：`clamp()`, `BigNumber.min()`, `BigNumber.max()`
...
| 公式引擎 | 双向 | BigNumber 用于绝对量，float 用于比值和乘数 | 资源量、属性值、伤害值使用 BigNumber；比值（如 attack/(attack+defense)）和百分比乘数使用 float |
| 修正器/倍率引擎 | 双向 | modifier 值为 float，通过 `multiply_float()` 应用到 BigNumber 基础值 | 加法 modifier 用 `add()`，乘法/百分比 modifier 用 `multiply_float()`，指数 modifier 用 `power()` |
| 资源系统 | 下游消费 | 所有资源值存储为 BigNumber | 资源的增减、上限检查、溢出检测 |
| 属性系统 | 下游消费 | 所有角色属性存储为 BigNumber | 攻击、防御、生命等属性的存储与计算 |
| 物品/材料系统（Alpha 阶段引入） | 下游消费（Alpha） | 物品价值、堆叠数量用 BigNumber | **MVP 不接入**——本系统在 MVP 阶段是纯元数据查询服务，无 BigNumber 字段。Alpha 阶段引入物品出售价格 / 库存累积量等字段时，才使用 BigNumber 存储 |
...
`multiply_float(a, f) → result`

**变量：**
| 变量 | 符号 | 类型 | 范围 | 说明 |
|------|------|------|------|------|
| a.mantissa | mₐ | float | [1.0, 10.0) | BigNumber 操作数有效数字 |
| a.exponent | eₐ | int | [0, 308] | BigNumber 操作数幂次 |
| f | f | float | [0.0, +∞) | 浮点乘数 |

**输出范围：** `BigNumber(0)` 到 `BigNumber(9.999, 308)`

**计算过程：**
1. 若 `f <= 0.0`：返回 `BigNumber.ZERO`
2. `result_m = mₐ × f`
3. `result_e = eₐ`
4. 返回 `normalize(result_m, result_e)`

**示例：** `multiply_float({2.0, 5}, 1.5)` → result_m = 3.0, result_e = 5 → `{3.0, 5}` = 300000
...
- **If 除法结果 < 1（如 5 ÷ 7）**：返回 `BigNumber.ZERO`。本系统仅表示 ≥ 1 的绝对量，比值计算应使用 float。此行为是设计决策，非缺陷。
...
| 公式引擎 | 下游依赖 BigNumber | 硬依赖 | 绝对量（资源、属性、伤害）使用 BigNumber；比值和百分比乘数使用 float |
| 修正器/倍率引擎 | 下游依赖 BigNumber | 硬依赖 | modifier 值为 float，通过 `multiply_float()` 应用到 BigNumber 基础值；加法 modifier 用 `add()` |
| 资源系统 | 下游依赖 BigNumber | 硬依赖 | 所有资源值（灵气、修为、灵石等）存储为 BigNumber |
| 属性系统 | 下游依赖 BigNumber | 硬依赖 | 所有角色属性（攻击、防御、生命等）存储为 BigNumber |
| 物品/材料系统（Alpha 引入） | 下游依赖（Alpha） | 软依赖（MVP 不接入） | **MVP 阶段不使用 BigNumber**——本系统是纯元数据查询服务（详见 item-material-system.md §Dependencies "关键非依赖"）。Alpha 阶段引入物品出售价格 / 库存累积量等字段时启用 BigNumber 存储 |
...
- [ ] **GIVEN** 1000 个 BigNumber 实例执行各 50 次加减乘除，**WHEN** 在单帧内完成，**THEN** 总耗时 < 16.6ms（60fps 帧预算内，纯 GDScript MVP 目标；若不达标，升级至 GDExtension C++）
...
- [ ] **GIVEN** `BigNumber.from_int(42)`，**WHEN** 创建实例，**THEN** `mantissa == 4.2`，`exponent == 1`
- [ ] **GIVEN** `BigNumber.from_float(3.14)`，**WHEN** 创建实例，**THEN** `mantissa` 约等于 `3.14`，`exponent == 0`
- [ ] **GIVEN** `a = BigNumber.MAX`，**WHEN** 调用 `a.is_max()`，**THEN** 返回 `true`
...
- [ ] **GIVEN** `a = {1.23, 150}`，**WHEN** 调用 `a.to_string()`，**THEN** 返回格式为 `"1.23e150"` 的字符串
- [ ] **GIVEN** `a = {2.0, 3}` 和 `b = {5.0, 3}`，**WHEN** 执行 `a.less_than(b)`，**THEN** 返回 `true`
- [ ] **GIVEN** `a = {5.0, 3}` 和 `b = {2.0, 3}`，**WHEN** 执行 `a.greater_than(b)`，**THEN** 返回 `true`
- [ ] **GIVEN** `a = {2.0, 3}` 和 `b = {2.0, 3}`，**WHEN** 执行 `a.less_or_equal(b)` 和 `a.greater_or_equal(b)`，**THEN** 均返回 `true`
- [ ] **GIVEN** `BigNumber.MAX` 常量，**WHEN** 读取其值，**THEN** `mantissa ∈ [9.998, 10.0)`，`exponent == 308`，`is_max()` 返回 `true`
- [ ] **GIVEN** `BigNumber.ONE` 常量，**WHEN** 读取其值，**THEN** `mantissa == 1.0`，`exponent == 0`
