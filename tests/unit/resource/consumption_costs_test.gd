...
func test_purchase_sufficient_gold() -> void:
	...
	var result: bool = _inventory.remove_resource(ResourceTypes.ResourceId.GOLD, 200)
	assert_true(result)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.GOLD), 300)

func test_purchase_insufficient_gold() -> void:
	_inventory.add_resource(ResourceTypes.ResourceId.GOLD, 100)
	var result: bool = _inventory.remove_resource(ResourceTypes.ResourceId.GOLD, 200)
	assert_false(result)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.GOLD), 100, "Gold unchanged")
...
func test_enhancement_deducts_resources() -> void:
	...
	_inventory.add_resource(ResourceTypes.ResourceId.BASIC_MATERIAL, 100)
	var cost: Dictionary = ResourceFormulas.calculate_enhancement_cost(100, 4)
	assert_true(_inventory.remove_resource(ResourceTypes.ResourceId.GOLD, cost["gold"]))
	assert_true(_inventory.remove_resource(ResourceTypes.ResourceId.BASIC_MATERIAL, cost["materials"]))
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.GOLD), 500)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.BASIC_MATERIAL), 75)

func test_insufficient_materials_prevents_enhancement() -> void:
	_inventory.add_resource(ResourceTypes.ResourceId.GOLD, 1000)
	_inventory.add_resource(ResourceTypes.ResourceId.BASIC_MATERIAL, 10)
	var cost: Dictionary = ResourceFormulas.calculate_enhancement_cost(100, 4)
	assert_false(_inventory.remove_resource(ResourceTypes.ResourceId.BASIC_MATERIAL, cost["materials"]),
...
func test_consume_fruit() -> void:
	...
	var result: bool = _inventory.remove_resource(ResourceTypes.ResourceId.FRUIT_STR, 1)
	assert_true(result)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.FRUIT_STR), 2)

func test_consume_fruit_insufficient() -> void:
	_inventory.add_resource(ResourceTypes.ResourceId.FRUIT_STR, 0)
	assert_false(_inventory.remove_resource(ResourceTypes.ResourceId.FRUIT_STR, 1))

func test_consume_fruit_correct_type() -> void:
	_inventory.add_resource(ResourceTypes.ResourceId.FRUIT_STR, 5)
	_inventory.add_resource(ResourceTypes.ResourceId.FRUIT_AGI, 5)
	_inventory.remove_resource(ResourceTypes.ResourceId.FRUIT_STR, 1)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.FRUIT_STR), 4)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.FRUIT_AGI), 5, "AGI fruit untouched")


# AC.3.4: Protection symbol

func test_protection_symbol_available() -> void:
	_inventory.add_resource(ResourceTypes.ResourceId.PROTECT_SYMBOL, 3)
	assert_true(_inventory.has_resource(ResourceTypes.ResourceId.PROTECT_SYMBOL, 1))

func test_consume_protection_symbol() -> void:
	_inventory.add_resource(ResourceTypes.ResourceId.PROTECT_SYMBOL, 1)
	_inventory.remove_resource(ResourceTypes.ResourceId.PROTECT_SYMBOL, 1)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.PROTECT_SYMBOL), 0)
...
func test_consume_barrier_resource() -> void:
	...
	var result: bool = _inventory.remove_resource(ResourceTypes.ResourceId.BARRIER_RESOURCE, 1)
	assert_true(result)
	assert_eq(_inventory.get_amount(ResourceTypes.ResourceId.BARRIER_RESOURCE), 0)

func test_insufficient_barrier_resource() -> void:
	assert_false(_inventory.remove_resource(ResourceTypes.ResourceId.BARRIER_RESOURCE, 1),
