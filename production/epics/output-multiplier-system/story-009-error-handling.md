...
- [x] AC-20: **GIVEN** `get_production_rate("lingqi")` 返回非零 float，**WHEN** `get_tick_amount("lingqi", 0.0)` 和 `get_tick_amount("lingqi", -1.0)` 分别被调用，**THEN** 两者均返回 `BigNumber.ZERO`，carry 不变，各打印一条 warning 提示 delta 无效
...
- Sprint execution order: Sprint 7, story 11/20
