...
- [ ] **AC-CH2-005.1**: Ch.2-3 普通路径：第 12 回合开始时，地图右侧出现 2 名 basic_knight 援军。
- [ ] **AC-CH2-005.2**: Boss 阶段三提前路径：Boss HP ≤ 30% 时当前回合为第 8 回合，援军在第 10 回合刷新（提前 2 回合）。
- [ ] **AC-CH2-005.3**: 援军出现后，胜利条件仍为"击败 Boss"（不需要同时击败援军）。
- [ ] **AC-CH2-006.1**: 陈朗 HP 降至 65% 以下时，系统保存检查点；Boss HP 以检查点保留值重置（见 BOSS-GDD-001 语义定义）。
- [ ] **AC-CH2-006.2**: 从检查点重开时，玩家己方单位 HP/状态与检查点一致（不重置为战斗开始值）。
