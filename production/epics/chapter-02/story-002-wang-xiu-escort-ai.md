...
- [ ] **AC-CH2-002.1**: 模拟 5 回合：每回合王秀的位置向安全区（左上 3×3）方向移动至少 1 格（无障碍情形）。
- [ ] **AC-CH2-002.2**: 模拟阻塞情形：敌方单位距王秀 ≤2 格时，王秀该回合不移动（畏缩生效）。
- [ ] **AC-CH2-004.1**: 王秀 HP 降至 0 时，触发退场剧情动画；`wang_xiu_departed = true` 写入存档；战斗不结束（可继续作战）。
- [ ] **AC-CH2-004.2**: 王秀退场后，战斗结算时信念值结算为 仁-8（不是 仁+12）。
