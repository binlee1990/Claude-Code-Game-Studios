...
- [x] GIVEN: `definition` 缺 `category` 字段，**WHEN** `register_entity`，**THEN** 返回 `false`，打印警告列出缺失字段
- [x] GIVEN: `"enemy_001"` 已注册含 6 属性，**WHEN** `unregister_entity("enemy_001")`，**THEN** 返回 `6`，`has_entity == false`，发布一条 `attribute.enemy_001.unregistered` 事件
...
- **AC**: GIVEN: `definition` 缺 `category` 字段，**WHEN** `register_entity`，**THEN** 返回 `false`，打印警告列出缺失字段
  - Given: `definition` 缺 `category` 字段
  - When: `register_entity`
  - Then: 返回 `false`，打印警告列出缺失字段
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `"enemy_001"` 已注册含 6 属性，**WHEN** `unregister_entity("enemy_001")`，**THEN** 返回 `6`，`has_entity == false`，发布一条 `attribute.enemy_001.unregistered` 事件
  - Given: `"enemy_001"` 已注册含 6 属性
  - When: `unregister_entity("enemy_001")`
  - Then: 返回 `6`，`has_entity == false`，发布一条 `attribute.enemy_001.unregistered` 事件
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Required evidence**:
- `tests/integration/attribute/2_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 5, story 15/20
