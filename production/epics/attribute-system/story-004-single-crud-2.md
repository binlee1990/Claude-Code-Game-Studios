...
- **AC**: GIVEN: `entity_id="ghost"` 未注册，**WHEN** `set_base("ghost", "atk", ...)`，**THEN** 拒绝写入，打印警告
  - Given: `entity_id="ghost"` 未注册
  - When: `set_base("ghost", "atk", ...)`
  - Then: 拒绝写入，打印警告
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `get_base("nonexistent", "atk")`，**WHEN** 调用，**THEN** 返回 `BigNumber.ZERO`，不打印警告（防御性读）
  - Given: `get_base("nonexistent", "atk")`
  - When: 调用
  - Then: 返回 `BigNumber.ZERO`，不打印警告（防御性读）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
