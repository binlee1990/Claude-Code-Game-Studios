...
- [x] GIVEN: `{9.9999, 7}`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.00亿"`（舍入跨单位）
- [x] GIVEN: `BigNumber.from_int(15000)`，**WHEN** 调用 `format_scientific()`，**THEN** 返回 `"1.50e4"`
- [x] GIVEN: `BigNumber.MAX`，**WHEN** 调用 `format_scientific()`，**THEN** 返回 `"MAX"`
...
- **AC**: GIVEN: `{9.9999, 7}`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.00亿"`（舍入跨单位）
  - Given: `{9.9999, 7}`
  - When: 调用 `format()`
  - Then: 返回 `"1.00亿"`（舍入跨单位）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.from_int(15000)`，**WHEN** 调用 `format_scientific()`，**THEN** 返回 `"1.50e4"`
  - Given: `BigNumber.from_int(15000)`
  - When: 调用 `format_scientific()`
  - Then: 返回 `"1.50e4"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.MAX`，**WHEN** 调用 `format_scientific()`，**THEN** 返回 `"MAX"`
  - Given: `BigNumber.MAX`
  - When: 调用 `format_scientific()`
  - Then: 返回 `"MAX"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/number_formatting/1-00_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 3, story 7/20
