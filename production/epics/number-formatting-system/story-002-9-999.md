...
- [x] GIVEN: `BigNumber.from_int(9999)`，**WHEN** 调用 `format()`，**THEN** 返回 `"9,999"`
- [x] GIVEN: `BigNumber.from_int(15000)`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.50万"`
- [x] GIVEN: `BigNumber.from_int(567800)`，**WHEN** 调用 `format()`，**THEN** 返回 `"56.8万"`
...
- **AC**: GIVEN: `BigNumber.from_int(9999)`，**WHEN** 调用 `format()`，**THEN** 返回 `"9,999"`
  - Given: `BigNumber.from_int(9999)`
  - When: 调用 `format()`
  - Then: 返回 `"9,999"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.from_int(15000)`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.50万"`
  - Given: `BigNumber.from_int(15000)`
  - When: 调用 `format()`
  - Then: 返回 `"1.50万"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.from_int(567800)`，**WHEN** 调用 `format()`，**THEN** 返回 `"56.8万"`
  - Given: `BigNumber.from_int(567800)`
  - When: 调用 `format()`
  - Then: 返回 `"56.8万"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/number_formatting/9-999_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 3, story 3/20
