...
- [x] GIVEN: `BigNumber.MAX`，**WHEN** 调用 `format()`，**THEN** 返回 `"MAX"`
- [x] GIVEN: `{1.0, 4}`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.00万"`
- [x] GIVEN: `{9.9999, 3}`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.00万"`（舍入跨边界）
...
- **AC**: GIVEN: `BigNumber.MAX`，**WHEN** 调用 `format()`，**THEN** 返回 `"MAX"`
  - Given: `BigNumber.MAX`
  - When: 调用 `format()`
  - Then: 返回 `"MAX"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `{1.0, 4}`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.00万"`
  - Given: `{1.0, 4}`
  - When: 调用 `format()`
  - Then: 返回 `"1.00万"`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `{9.9999, 3}`，**WHEN** 调用 `format()`，**THEN** 返回 `"1.00万"`（舍入跨边界）
  - Given: `{9.9999, 3}`
  - When: 调用 `format()`
  - Then: 返回 `"1.00万"`（舍入跨边界）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/number_formatting/max_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 3, story 6/20
