# Story 002: 返回含 3 个键的 Dictionary
...
- [x] GIVEN: 表 `enemies` 含 3 条记录，**WHEN** 执行 `DataConfig.get_all("enemies")`，**THEN** 返回含 3 个键的 Dictionary
- [x] GIVEN: 表 `items` 含 10 条记录（5 个 level >= 5），**WHEN** 执行 `DataConfig.query("items", func(r): return r["level"] >= 5)`，**THEN** 返回 5 条记录的数组
- [x] GIVEN: `enemies.json` 文件不存在，**WHEN** 执行 `load_all()`，**THEN** `has_table("enemies")` 返回 `false`，其他表正常加载
...
- **AC**: GIVEN: 表 `enemies` 含 3 条记录，**WHEN** 执行 `DataConfig.get_all("enemies")`，**THEN** 返回含 3 个键的 Dictionary
  - Given: 表 `enemies` 含 3 条记录
  - When: 执行 `DataConfig.get_all("enemies")`
  - Then: 返回含 3 个键的 Dictionary
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: 表 `items` 含 10 条记录（5 个 level >= 5），**WHEN** 执行 `DataConfig.query("items", func(r): return r["level"] >= 5)`，**THEN** 返回 5 条记录的数组
  - Given: 表 `items` 含 10 条记录（5 个 level >= 5）
  - When: 执行 `DataConfig.query("items", func(r): return r["level"] >= 5)`
  - Then: 返回 5 条记录的数组
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `enemies.json` 文件不存在，**WHEN** 执行 `load_all()`，**THEN** `has_table("enemies")` 返回 `false`，其他表正常加载
  - Given: `enemies.json` 文件不存在
  - When: 执行 `load_all()`
  - Then: `has_table("enemies")` 返回 `false`，其他表正常加载
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
- Sprint execution order: Sprint 3, story 11/20
