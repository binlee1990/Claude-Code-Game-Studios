...
- **AC**: GIVEN: 某表 JSON 语法错误，**WHEN** 执行 `load_all()`，**THEN** 该表为空，其他表正常加载，打印错误含文件路径
  - Given: 某表 JSON 语法错误
  - When: 执行 `load_all()`
  - Then: 该表为空，其他表正常加载，打印错误含文件路径
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: 表 `enemies` 含 `{"slime": {"name": "史莱姆", "hp": "100"}}`，**WHEN** 执行 `DataConfig.get_field("enemies", "slime", "hp")`，**THEN** 返回 `"100"`（字符串类型保持原样）
  - Given: 表 `enemies` 含 `{"slime": {"name": "史莱姆", "hp": "100"}}`
  - When: 执行 `DataConfig.get_field("enemies", "slime", "hp")`
  - Then: 返回 `"100"`（字符串类型保持原样）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: 记录中不含字段 `mp`，**WHEN** 执行 `DataConfig.get_field("enemies", "slime", "mp")`，**THEN** 返回 `null`，不打印警告
  - Given: 记录中不含字段 `mp`
  - When: 执行 `DataConfig.get_field("enemies", "slime", "mp")`
  - Then: 返回 `null`，不打印警告
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
