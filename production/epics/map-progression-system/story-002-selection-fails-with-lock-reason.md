# Story 002: selection fails with lock reason
...
- **AC**: GIVEN: a locked zone is selected, **WHEN** selection is attempted, **THEN** selection fails with lock reason.
  - Given: a locked zone is selected
  - When: selection is attempted
  - Then: selection fails with lock reason
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: save/load roundtrip, **WHEN** progress restores, **THEN** unlocked/cleared states match saved data.
  - Given: save/load roundtrip
  - When: progress restores
  - Then: unlocked/cleared states match saved data
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Required evidence**:
- `tests/integration/map_progression/selection-fails-with-lock-reason_test.gd` — must exist and pass
