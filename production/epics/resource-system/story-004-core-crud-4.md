...
- [x] GIVEN: 5 资源已注册，changes=`{lingqi: 100, herb: 50, exp: 200}`，**WHEN** `batch_add(changes)`，**THEN** 返回 Dictionary 含 3 键，每条 actual_added 与预期一致
...
- **AC**: GIVEN: 5 资源已注册，changes=`{lingqi: 100, herb: 50, exp: 200}`，**WHEN** `batch_add(changes)`，**THEN** 返回 Dictionary 含 3 键，每条 actual_added 与预期一致
  - Given: 5 资源已注册，changes=`{lingqi: 100, herb: 50, exp: 200}`
  - When: `batch_add(changes)`
  - Then: 返回 Dictionary 含 3 键，每条 actual_added 与预期一致
...
**Required evidence**:
- `tests/integration/resource/core-crud-4_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 5, story 4/20
