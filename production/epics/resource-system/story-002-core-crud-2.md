...
- [x] GIVEN: `lingqi` current=`BigNumber.from_int(800)`，cap=`BigNumber.from_int(1000)`，**WHEN** `add("lingqi", BigNumber.from_int(300))`，**THEN** 返回 `BigNumber.from_int(200)`（钳位），`get_value == BigNumber.from_int(1000)`
- [x] GIVEN: `lingshi` current=`BigNumber.from_int(500)`（has_cap=false），**WHEN** `add("lingshi", BigNumber.from_int(9999))`，**THEN** 返回 `BigNumber.from_int(9999)`，`get_value == BigNumber.from_int(10499)`
- [x] GIVEN: `herb` current=`BigNumber.from_int(300)`，**WHEN** `spend("herb", BigNumber.from_int(300))`，**THEN** 返回 `true`，`get_value == BigNumber.ZERO`
...
- Sprint execution order: Sprint 5, story 2/20
