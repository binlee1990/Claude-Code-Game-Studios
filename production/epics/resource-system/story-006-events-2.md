...
- [x] GIVEN: `add` 或 `spend` 的 amount=`BigNumber.ZERO`，**WHEN** 调用，**THEN** `get_value` 不变，EventBus 不发布任何事件
- [x] GIVEN: `herb` current=`BigNumber.from_int(100)`，**WHEN** `spend("herb", BigNumber.from_int(100))`，**THEN** 返回 true，发布 `changed`（new=ZERO），**不**发布 overflow
...
- **AC**: GIVEN: `add` 或 `spend` 的 amount=`BigNumber.ZERO`，**WHEN** 调用，**THEN** `get_value` 不变，EventBus 不发布任何事件
  - Given: `add` 或 `spend` 的 amount=`BigNumber.ZERO`
  - When: 调用
  - Then: `get_value` 不变，EventBus 不发布任何事件
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `herb` current=`BigNumber.from_int(100)`，**WHEN** `spend("herb", BigNumber.from_int(100))`，**THEN** 返回 true，发布 `changed`（new=ZERO），**不**发布 overflow
  - Given: `herb` current=`BigNumber.from_int(100)`
  - When: `spend("herb", BigNumber.from_int(100))`
  - Then: 返回 true，发布 `changed`（new=ZERO），不发布 overflow
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Required evidence**:
- `tests/integration/resource/events-2_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 5, story 6/20
