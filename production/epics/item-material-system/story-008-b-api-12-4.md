...
- **AC**: AC-B10: GIVEN herb.tags=`["herb","low_tier"]`，WHEN `query_by_tag("herb")`，THEN herb 在返回数组中（contains 语义，不是精确匹配）
  - Given: herb.tags=`["herb","low_tier"]`
  - When: `query_by_tag("herb")`
  - Then: herb 在返回数组中（contains 语义，不是精确匹配）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: AC-B11: GIVEN herb.tags=`["herb","low_tier"]`，WHEN `query_by_tag("nonexistent_tag")`，THEN `[]`，不打印警告
  - Given: herb.tags=`["herb","low_tier"]`
  - When: `query_by_tag("nonexistent_tag")`
  - Then: `[]`，不打印警告
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: AC-B12: GIVEN ItemRegistry 已加载多条带 tags 的物品，WHEN `query_by_tag("")` 空字符串，THEN `[]`（无任何 record.tags 含空字符串），不打印警告
  - Given: ItemRegistry 已加载多条带 tags 的物品
  - When: `query_by_tag("")` 空字符串
  - Then: `[]`（无任何 record.tags 含空字符串），不打印警告
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
