...
- [x] AC-A4: GIVEN items.json 含 `item_class="unknown_cat"` 的非法记录，WHEN 加载，THEN 该条不在 `_items` 中 + 警告，其他记录正常加载
- [x] AC-A5: GIVEN items.json 含 `name=""` 的记录，WHEN 加载，THEN 该条被跳过 + 警告
- [x] AC-A6: GIVEN items.json 含 `rarity="unknown_rarity"` 的记录，WHEN 加载，THEN rarity 降级为 `"fanpin"` + 警告，条目仍被注册
...
- **AC**: AC-A4: GIVEN items.json 含 `item_class="unknown_cat"` 的非法记录，WHEN 加载，THEN 该条不在 `_items` 中 + 警告，其他记录正常加载
  - Given: items.json 含 `item_class="unknown_cat"` 的非法记录
  - When: 加载
  - Then: 该条不在 `_items` 中 + 警告，其他记录正常加载
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: AC-A5: GIVEN items.json 含 `name=""` 的记录，WHEN 加载，THEN 该条被跳过 + 警告
  - Given: items.json 含 `name=""` 的记录
  - When: 加载
  - Then: 该条被跳过 + 警告
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: AC-A6: GIVEN items.json 含 `rarity="unknown_rarity"` 的记录，WHEN 加载，THEN rarity 降级为 `"fanpin"` + 警告，条目仍被注册
  - Given: items.json 含 `rarity="unknown_rarity"` 的记录
  - When: 加载
  - Then: rarity 降级为 `"fanpin"` + 警告，条目仍被注册
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
- Sprint execution order: Sprint 6, story 7/20
