...
- [x] AC-A9: GIVEN items.json 含某记录 `tags=null` 或 `tags="herb"`（非 Array），WHEN 加载，THEN tags 降级为 `[]` + 警告，条目仍被注册
- [x] AC-A10: 参数化测试 — GIVEN mock items 含 8 条记录，每条 `rarity` 分别为 `fanpin / jingliang / xiyou / shishi / chuanshuo / shenhua / xiantian / hundun`，其余字段（id/name/item_class）合法且各异，WHEN 加载，THEN `get_count() == 8` 且对每条记录 `peek_field(id, "rarity")` 返回 items.json 输入的原值（无降级）。**目的**：防止 8 元枚举校验列表的拼写漏洞（如把 `xiantian` 拼为 `xiātian` 或漏掉 `hundun`）静默失效
...
- Sprint execution order: Sprint 6, story 9/20
