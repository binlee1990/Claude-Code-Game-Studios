...
- **AC**: GIVEN: offline 3600s and average cycle 10s, **WHEN** simulation runs in expected mode, **THEN** encounter count is 360.
  - Given: offline 3600s and average cycle 10s
  - When: simulation runs in expected mode
  - Then: encounter count is 360
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: no active zone, **WHEN** simulation runs, **THEN** result has zero rewards and warning.
  - Given: no active zone
  - When: simulation runs
  - Then: result has zero rewards and warning
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: same context and seed, **WHEN** short offline simulation runs twice, **THEN** outputs match.
  - Given: same context and seed
  - When: short offline simulation runs twice
  - Then: outputs match
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
