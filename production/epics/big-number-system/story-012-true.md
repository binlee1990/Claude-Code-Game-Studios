...
- [x] GIVEN: `a = {2.0, 3}` 和 `b = {2.0, 3}`，**WHEN** 执行 `a.less_or_equal(b)` 和 `a.greater_or_equal(b)`，**THEN** 均返回 `true`
- [x] GIVEN: `BigNumber.MAX` 常量，**WHEN** 读取其值，**THEN** `mantissa ∈ [9.998, 10.0)`，`exponent == 308`，`is_max()` 返回 `true`
- [x] GIVEN: `BigNumber.ONE` 常量，**WHEN** 读取其值，**THEN** `mantissa == 1.0`，`exponent == 0`
...
- **AC**: GIVEN: `a = {2.0, 3}` 和 `b = {2.0, 3}`，**WHEN** 执行 `a.less_or_equal(b)` 和 `a.greater_or_equal(b)`，**THEN** 均返回 `true`
  - Given: `a = {2.0, 3}` 和 `b = {2.0, 3}`
  - When: 执行 `a.less_or_equal(b)` 和 `a.greater_or_equal(b)`
  - Then: 均返回 `true`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.MAX` 常量，**WHEN** 读取其值，**THEN** `mantissa ∈ [9.998, 10.0)`，`exponent == 308`，`is_max()` 返回 `true`
  - Given: `BigNumber.MAX` 常量
  - When: 读取其值
  - Then: `mantissa ∈ [9.998, 10.0)`，`exponent == 308`，`is_max()` 返回 `true`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.ONE` 常量，**WHEN** 读取其值，**THEN** `mantissa == 1.0`，`exponent == 0`
  - Given: `BigNumber.ONE` 常量
  - When: 读取其值
  - Then: `mantissa == 1.0`，`exponent == 0`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/big_number/true_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 1, story 12/20
