...
- [x] GIVEN: `a = BigNumber.MAX`，**WHEN** 执行 `a.add(BigNumber.ONE)`，**THEN** 结果为 `BigNumber.MAX`（饱和）
- [x] GIVEN: 两个操作数 exponent 差 > 15，**WHEN** 执行加法，**THEN** 结果等于较大操作数
- [x] GIVEN: 两个相等值的 BigNumber，**WHEN** 执行 `a.equals(b)`，**THEN** 结果为 `true`
...
- Sprint execution order: Sprint 1, story 7/20
