...
- [x] GIVEN: `a = {3.5, 100}`，**WHEN** 调用 `a.magnitude()`，**THEN** 返回 `100`
- [x] GIVEN: `a = {4.2, 1}`，**WHEN** 调用 `a.to_int()`，**THEN** 返回 `42`
- [x] GIVEN: `a = {3.14, 0}`，**WHEN** 调用 `a.to_float()`，**THEN** 返回约 `3.14`（float）
...
- Sprint execution order: Sprint 1, story 10/20
