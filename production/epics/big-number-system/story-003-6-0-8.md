...
- [x] GIVEN: `a = {2.0, 5}` 和 `b = {3.0, 3}`，**WHEN** 执行 `a.multiply(b)`，**THEN** 结果为 `{6.0, 8}`
- [x] GIVEN: `a = {6.0, 8}` 和 `b = {2.0, 5}`，**WHEN** 执行 `a.divide(b)`，**THEN** 结果为 `{3.0, 3}`
- [x] GIVEN: `a = {2.0, 10}`，**WHEN** 执行 `a.power(2.0)`，**THEN** 结果为 `{4.0, 20}`
...
- **AC**: GIVEN: `a = {2.0, 5}` 和 `b = {3.0, 3}`，**WHEN** 执行 `a.multiply(b)`，**THEN** 结果为 `{6.0, 8}`
  - Given: `a = {2.0, 5}` 和 `b = {3.0, 3}`
  - When: 执行 `a.multiply(b)`
  - Then: 结果为 `{6.0, 8}`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `a = {6.0, 8}` 和 `b = {2.0, 5}`，**WHEN** 执行 `a.divide(b)`，**THEN** 结果为 `{3.0, 3}`
  - Given: `a = {6.0, 8}` 和 `b = {2.0, 5}`
  - When: 执行 `a.divide(b)`
  - Then: 结果为 `{3.0, 3}`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `a = {2.0, 10}`，**WHEN** 执行 `a.power(2.0)`，**THEN** 结果为 `{4.0, 20}`
  - Given: `a = {2.0, 10}`
  - When: 执行 `a.power(2.0)`
  - Then: 结果为 `{4.0, 20}`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/big_number/6-0-8_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 1, story 3/20
