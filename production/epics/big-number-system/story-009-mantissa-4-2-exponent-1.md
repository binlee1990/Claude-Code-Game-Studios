# Story 009: `mantissa == 4.2`，`exponent == 1`
...
- [x] GIVEN: `BigNumber.from_int(42)`，**WHEN** 创建实例，**THEN** `mantissa == 4.2`，`exponent == 1`
- [x] GIVEN: `BigNumber.from_float(3.14)`，**WHEN** 创建实例，**THEN** `mantissa` 约等于 `3.14`，`exponent == 0`
- [x] GIVEN: `a = BigNumber.MAX`，**WHEN** 调用 `a.is_max()`，**THEN** 返回 `true`
...
- **AC**: GIVEN: `BigNumber.from_int(42)`，**WHEN** 创建实例，**THEN** `mantissa == 4.2`，`exponent == 1`
  - Given: `BigNumber.from_int(42)`
  - When: 创建实例
  - Then: `mantissa == 4.2`，`exponent == 1`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.from_float(3.14)`，**WHEN** 创建实例，**THEN** `mantissa` 约等于 `3.14`，`exponent == 0`
  - Given: `BigNumber.from_float(3.14)`
  - When: 创建实例
  - Then: `mantissa` 约等于 `3.14`，`exponent == 0`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `a = BigNumber.MAX`，**WHEN** 调用 `a.is_max()`，**THEN** 返回 `true`
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/big_number/mantissa-4-2-exponent-1_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 1, story 9/20
