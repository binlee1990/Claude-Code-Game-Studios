# Story 008: 总耗时 < 16.6ms（60fps 帧预算内，纯 GDScript MVP 目标；若不达标，升级至 GDExtension C++）
...
- [x] GIVEN: 1000 个 BigNumber 实例执行各 50 次加减乘除，**WHEN** 在单帧内完成，**THEN** 总耗时 < 16.6ms（60fps 帧预算内，纯 GDScript MVP 目标；若不达标，升级至 GDExtension C++）
- [x] GIVEN: `BigNumber.from_int(0)`，**WHEN** 创建零值实例，**THEN** `is_zero()` 返回 `true`，`mantissa == 0.0`，`exponent == 0`
- [x] GIVEN: `a = {2.0, 3}` 且 `n = 0`，**WHEN** 执行 `a.power(n)`，**THEN** 结果为 `BigNumber.ONE`
...
- **AC**: GIVEN: 1000 个 BigNumber 实例执行各 50 次加减乘除，**WHEN** 在单帧内完成，**THEN** 总耗时 < 16.6ms（60fps 帧预算内，纯 GDScript MVP 目标；若不达标，升级至 GDExtension C++）
  - Given: 1000 个 BigNumber 实例执行各 50 次加减乘除
  - When: 在单帧内完成
  - Then: 总耗时 < 16.6ms（60fps 帧预算内，纯 GDScript MVP 目标；若不达标，升级至 GDExtension C++）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `BigNumber.from_int(0)`，**WHEN** 创建零值实例，**THEN** `is_zero()` 返回 `true`，`mantissa == 0.0`，`exponent == 0`
  - Given: `BigNumber.from_int(0)`
  - When: 创建零值实例
  - Then: `is_zero()` 返回 `true`，`mantissa == 0.0`，`exponent == 0`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `a = {2.0, 3}` 且 `n = 0`，**WHEN** 执行 `a.power(n)`，**THEN** 结果为 `BigNumber.ONE`
  - Given: `a = {2.0, 3}` 且 `n = 0`
  - When: 执行 `a.power(n)`
  - Then: 结果为 `BigNumber.ONE`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Story Type**: Logic
**Required evidence**:
- `tests/unit/big_number/16-6ms-60fps-gdscript-mvp-gdextension-c_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 1, story 8/20
