...
- **AC**: GIVEN: `a = {3.16, 5}`，**WHEN** 执行 `a.log10()`，**THEN** 结果约为 5.5（float，误差 < 0.01）
  - Given: `a = {3.16, 5}`
  - When: 执行 `a.log10()`
  - Then: 结果约为 5.5（float，误差 < 0.01）
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: `b = BigNumber.ZERO`，**WHEN** 执行 `a.divide(b)`，**THEN** 结果为 `BigNumber.MAX`
  - Given: `b = BigNumber.ZERO`
  - When: 执行 `a.divide(b)`
  - Then: 结果为 `BigNumber.MAX`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: 运算结果使 exponent > 308，**WHEN** 归一化执行，**THEN** 结果钳位为 `{9.999, 308}`
  - Given: 运算结果使 exponent > 308
  - When: 归一化执行
  - Then: 结果钳位为 `{9.999, 308}`
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
