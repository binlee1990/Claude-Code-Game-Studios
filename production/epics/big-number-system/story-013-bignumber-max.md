...
- [x] GIVEN: `a = BigNumber.ZERO` 且 `b = BigNumber.ZERO`，**WHEN** 执行 `a.divide(b)`，**THEN** 结果为 `BigNumber.MAX`
- [x] GIVEN: `a = BigNumber.ZERO`，**WHEN** 执行 `a.power(5.0)`，**THEN** 结果为 `BigNumber.ZERO`
- [x] GIVEN: `NaN` 作为参数，**WHEN** 执行 `BigNumber.from_float(NaN)`，**THEN** 结果为 `BigNumber.ZERO`
...
- Sprint execution order: Sprint 1, story 13/20
