...
- [x] GIVEN: FormulaEngine 中 `level_exp` 公式未注册（缺失），**WHEN** `gain_exp("player", BN(1000))`，**THEN** evaluate 返回 0.0 → 触发 MAX_LEVELS_PER_GAIN 截断保护，返回 100，`amount_remaining` 退回，打印警告
- [x] GIVEN: Lv.200（合体境上限），**WHEN** `gain_exp("player", BN(1e8))`，**THEN** 返回 0，level 仍 200，全部 amount 退回，不发事件
...
- Sprint execution order: Sprint 8, story 18/20
