...
- [x] GIVEN: `"ghost"` 未注册，**WHEN** `gain_exp("ghost", ...)`，**THEN** 返回 0，警告，ResourceSystem.exp 不变
- [x] GIVEN: Lv.1，exp=1e18，**WHEN** `gain_exp(BN.from_string("1e18"))`，**THEN** 返回 100（MAX_LEVELS_PER_GAIN 截断），amount_remaining 全量退回，警告打印
...
- **AC**: GIVEN: `"ghost"` 未注册，**WHEN** `gain_exp("ghost", ...)`，**THEN** 返回 0，警告，ResourceSystem.exp 不变
  - Given: `"ghost"` 未注册
  - When: `gain_exp("ghost", ...)`
  - Then: 返回 0，警告，ResourceSystem.exp 不变
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section

- **AC**: GIVEN: Lv.1，exp=1e18，**WHEN** `gain_exp(BN.from_string("1e18"))`，**THEN** 返回 100（MAX_LEVELS_PER_GAIN 截断），amount_remaining 全量退回，警告打印
  - Given: Lv.1，exp=1e18
  - When: `gain_exp(BN.from_string("1e18"))`
  - Then: 返回 100（MAX_LEVELS_PER_GAIN 截断），amount_remaining 全量退回，警告打印
  - Edge cases: boundary, invalid, missing-data, and repeat-run variants from the GDD edge-case section
...
**Required evidence**:
- `tests/integration/level/gain-exp-2_test.gd` — must exist and pass
...
- Sprint execution order: Sprint 8, story 8/20
