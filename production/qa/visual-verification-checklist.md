...
当前 Sprint 1-3 自动化 QA 已签核：`tests/test_runner.gd` 报告 `Total Passed: 253`，且 `SCRIPT ERROR` / `Assertion failed` / `ERROR:` / `WARNING:` 均为 0。`src/Game.tscn` headless scene boot 也为 clean。
...
> **人工反馈刷新（2026-05-02）**: 已根据 CP1/CP2/CP3/CP4 反馈更新勾选状态。已确认正确的项标为 `[x]`；已修复但尚未人工复测的项保留 `[ ]` 并标注“已修复，待复测”。
...
### 验证
- [ ] 单位**瞬移**到目标位置（无动画）— 最新反馈：点击可达格没有效果；已修复，待复测
- [ ] 蓝色移动高亮**全部清除** — 已修复，待复测
- [ ] 单位射程内（rng=1）的敌方瓦片显示**橙色矩形** (#EA580C) — 已修复，待复测
- [ ] 如果射程内无敌方单位 → 不显示橙色高亮，单位自动结束行动 — 已修复，待复测
...
- [ ] 颜色规则：
  ...
  - 伤害 ≥ 目标 HP → **红色** (#EF4444)
- [ ] 鼠标移开后数字**立即消失**
- [ ] 伤害计算正确：Player atk=5, Enemy def=1 → 显示 "-4"（琥珀色，4<8）
...
### 验证
- [ ] 敌方 HP 标签**立即更新**：从 "HP: 8/8" 变为 "HP: 4/8"
- [ ] "-4" 伤害数字停留约 **0.6 秒**后消失
- [ ] 攻击者单位变为已行动状态
- [ ] Player 单位 HP **不变**（无反击）
...
| CP4 移动+橙高亮 | ◐ | 人工反馈为点击可达瓦片没有效果；已修复 Map.move_unit 同步 Unit 世界坐标，并补 CP4 自动化回归，待人工复测。 |
| CP5 伤害预览 | ⬜ | |
| CP6 攻击+击杀 | ⬜ | |
| CP7 End Turn | ⬜ | |
| CP8a WIN | ⬜ | |
