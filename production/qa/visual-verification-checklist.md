...
当前 Sprint 1-3 自动化 QA 已签核：`tests/test_runner.gd` 报告 `Total Passed: 254`，且 `SCRIPT ERROR` / `Assertion failed` / `ERROR:` / `WARNING:` 均为 0。`src/Game.tscn` headless scene boot 也为 clean。
...
> **人工反馈刷新（2026-05-02）**: 已根据 CP1-CP6 反馈更新勾选状态。已确认正确的项标为 `[x]`；已修复但尚未人工复测的项保留 `[ ]` 并标注“已修复，待复测”。
...
- [ ] 颜色规则：
  ...
  - [ ] 伤害 ≥ 目标 HP → **红色** (#EF4444)，带黑色描边以避免和 Enemy 红色方块混淆 — 已增强，待复测
- [x] 鼠标移开后数字**立即消失**
- [ ] 伤害计算正确：预览显示 "-4" 与 "ATK 5 - DEF 1"，用于现场校验公式 — 已增强，待复测
...
### 验证
- [ ] 敌方 HP 标签**立即更新**：从 "HP: 8/8" 变为 "HP: 4/8" — 已修复，待复测
- [ ] "-4" 伤害数字停留约 **0.6 秒**后消失 — 待复测
- [ ] 攻击者单位变为已行动状态（半透明灰色）— 已修复即时刷新，待复测
...
### 验证
- [ ] 敌方 HP 从 4/8 降至 0（标签显示 "HP: 0/8"）
- [ ] 敌方单位**从棋盘完全消失**（queue_free）
- [ ] 死亡位置变为空瓦片（可被其他单位移动至此）
...
### 验证
- [ ] HUD 阵营指示器从 "Player Turn"（蓝色）切换为 **"Enemy Turn"**（红色）
- [ ] Turn 数字不变（Player→Enemy 不递增回合）
- [ ] 敌方单位 `has_acted_this_turn` 重置为 false（可被选中）
...
- [ ] 蓝色移动范围出现
- [ ] 可以移动/攻击 Player 单位
- [ ] 再次点击 End Turn → 切换回 Player
- [ ] Turn 从 1 变为 2（Enemy→Player 时递增）
...
### 验证
- [ ] 最后一个 Enemy 死亡的同一帧，**全屏半透明深色背景**出现
- [ ] 标题：绿色大字 **"VICTORY"** (#10B981)
- [ ] 原因文字：显示 "elimination"
- [ ] **"Play Again"** 按钮可见可点击
- [ ] 所有输入被屏蔽（MOUSE_FILTER_STOP）
...
### 验证
- [ ] 最后一个 Player 死亡瞬间，全屏覆盖层出现
- [ ] 标题：红色大字 **"DEFEAT"** (#EF4444)
- [ ] 原因文字：显示 "elimination"
- [ ] "Play Again" 按钮可见
...
- [ ] 运行全过程无控制台报错（红色 SCRIPT ERROR）
- [ ] 帧率保持流畅（无卡顿）
- [ ] 内存无明显泄漏（任务管理器观察，5 分钟内增长不超过 50MB）
- [ ] 窗口缩放/最小化后恢复正常渲染
...
| CP5 伤害预览 | ◐ | 人工反馈确认数字、琥珀色、移出清除正常；已增强致命红色描边和 ATK/DEF 公式显示，待复测。 |
| CP6 攻击+击杀 | ◐ | 人工反馈确认无反击；已修复 HP 标签和已行动灰色状态即时刷新，待复测攻击后反馈。 |
| CP7 End Turn | ⬜ | |
| CP8a WIN | ⬜ | |
| CP8b DEFEAT | ⬜ | |
| CP10 DRAW | ⬜ | |
